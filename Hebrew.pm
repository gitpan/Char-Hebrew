...
my $qq_substr  = qr{(?: Hebrew::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $function_ord;         # ord()   to ord()   or Hebrew::ord()
my $function_ord_;        # ord     to ord     or Hebrew::ord_
my $function_reverse;     # reverse to reverse or Hebrew::reverse
my $function_getc;        # getc    to getc    or Hebrew::getc
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Hebrew::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Hebrew::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Hebrew::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Hebrew::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Hebrew::re_t=%s; while($Hebrew::re_t =~ %s){%s local $^W=0; local $Hebrew::re_r=%s; %s$Hebrew::re_t="$`$Hebrew::re_r$'"; pos($Hebrew::re_t)=length "$`$Hebrew::re_r"; } return $Hebrew::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Hebrew::re_r=eval $Hebrew::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Hebrew::re_r=%s; %s"%s$Hebrew::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Hebrew::re_n=0; while(%s =~ %s){%s local $^W=0; local $Hebrew::re_r=%s; %s%s="$`$Hebrew::re_r$'"; pos(%s)=length "$`$Hebrew::re_r"; $Hebrew::re_n++} return %s$Hebrew::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Hebrew::re_r=%s; %s%s="%s$Hebrew::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Hebrew::re_r=%s; %s%s="%s$Hebrew::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Hebrew::re_r=eval $Hebrew::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
expand wildcard arguments supplied to programs. Instead, import of Ehebrew.pm
works well.
...
=head1 Calling 'Ehebrew::ignorecase()' (Hebrew.pm provides)

Hebrew.pm applies calling 'Ehebrew::ignorecase()' instead of /i modifier.
...
Definitions in Ehebrew.pm.
  ...
  ${Ehebrew::not_lower_i}    qr{(?:[\x00-\xFF])}
  ...
  ${Ehebrew::not_upper_i}    qr{(?:[\x00-\xFF])}
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Hebrew.pm provides)

Hebrew.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Hebrew.pm and Ehebrew.pm provide)
...
=head1 Un-Escaping bytes::* Subroutines (Hebrew.pm provides)

Hebrew.pm removes 'bytes::' at head of subroutine name.
...
You need copy built-in standard module to /Perl/site/lib/Hebrew and change
'use utf8;' to 'use Hebrew;' in its. You need help yourself for now.
